...
目前，编写自动化测试还不是很直接-例如为 Truffle、 Ethereum 中的 Embark 可用的对象-特别是以 Substrate为基础的链上的特定区块链功能编写。一个测试套件允许在套件中植入测试钱包，并以确定性的方式执行状态测试。输出应该是一个完整的类别，包括问题和如何修复它们的建议(如果已知修复方式)。理想情况下，错误修复过程应该是一项众包工作。在此之前，未被发现的错误将自动转化为问题和配方中央存储库中的新类别。
