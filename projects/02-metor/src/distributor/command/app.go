...
const (
	logo = `  __  __  _____  _____  ___   ____  
 |  \/  || ____||_   _|/ _ \ |  _ \ 
 | |\/| ||  _|    | | | | | || |_) |
 | |  | || |___   | | | |_| ||  _ < 
 |_|  |_||_____|  |_|  \___/ |_| \_\`
...
func (_this *App) Run() {
	...
	cmd.PersistentFlags().StringVarP(&keystorePath, "keystore", "k", config.Api.Home+"keystore", "keystore path")
...
func (_this *App) start(cmd *cobra.Command, args []string) {
	...
	//todo 测试完删除这个根节点模式
	if !cast.ToBool(rootMode) {
		config.Api.P2PHost = []string{"/ip4/0.0.0.0/tcp/0"}
	}
