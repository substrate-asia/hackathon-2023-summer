...
func (_this *Libp2p) GetBlockResp(stream network.Stream) {
	defer ihelp.ErrCatch()
	...
	var fromPubKey string
	if fromPubKey, err = _this.Account.VerifySign(data.Data, data.Signature); err != nil {
		ilog.Logger.Error(err)
		return
	}
	...
	blockChan, ok := _this.chanMap.Load(fromPubKey + hash)
	if !ok {
		ilog.Logger.Warnf("Abnormal data received [%s %s]", fromPubKey, hash)
		return
	}
