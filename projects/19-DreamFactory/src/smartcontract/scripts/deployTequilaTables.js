...
async function main() {
  ...
  // Get the contract factory to create an instance of the  DreamFactoryNFT contract
  const DreamFactoryNFT = await ethers.getContractFactory("DreamFactoryNFT");
  ...
  const dreamfactoryNFT = await DreamFactoryNFT.deploy(
    tablelandBaseURI,
    mainName,
    attributesName
  );
  // For contract verification purposes, wait for 5 confirmations before proceeeding
  // Otherwise, just use `await dreamfactoryNFT.deployed()`
  await dreamfactoryNFT.deployTransaction.wait(5);

  // Log the deployed address and call the getter on `baseURIString` (for demonstration purposes)
  console.log(
    `\DreamFactoryNFT contract deployed on ${network.name} at: ${dreamfactoryNFT.address}`
  );
  const baseURI = await dreamfactoryNFT.baseURIString();
  console.log(`DreamFactoryNFT is using baseURI: ${baseURI}`);

  // For demonstration purposes, mint a token so that `tokenURI` can be called
  const mintToken = await dreamfactoryNFT.mint();
  ...
  const tokenURI = await dreamfactoryNFT.tokenURI(tokenId);
  console.log(
  ...
  try {
    ...
    await hre.run("verify:verify", {
      address: dreamfactoryNFT.address,
      constructorArguments: [tablelandBaseURI, mainName, attributesName],
    });
  } catch (err) {
    if (err.message.includes("Reason: Already Verified")) {
      console.log(
        `Contract is already verified! Check it out on Polygonscan: https://mumbai.polygonscan.com/address/${dreamfactoryNFT.address}`
      );
    }
  }
}
