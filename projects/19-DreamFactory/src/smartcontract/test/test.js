...
describe("dreamfactoryNFT", function () {
  let DreamFactoryNFT;
  let dreamfactoryNFT;
  let owner;
  let addr1;
  ...
  before(async () => {
    ...
    DreamFactoryNFT = await ethers.getContractFactory("DreamFactoryNFT");
    dreamfactoryNFT = await DreamFactoryNFT.deploy(
      tablelandGatewayURI,
      mainTable,
      attributesTable
    );
    await dreamfactoryNFT.deployed();
  });
  it("Should mint a token to the caller address and allow a transfer", async function () {
    ...
    const mintTx = await dreamfactoryNFT.connect(owner).mint();
    await mintTx.wait();

    // Validate the owner of the minted token is `owner`
    expect(await dreamfactoryNFT.ownerOf(0)).to.equal(owner.address);
    // Validate the NFT can be transferred
    await dreamfactoryNFT["safeTransferFrom(address,address,uint256)"](
      owner.address,
      addr1.address,
      0
    );
    expect(await dreamfactoryNFT.ownerOf(0)).to.equal(addr1.address);
  });
  it("Should return `tokenURI` for a valid `tokenId`", async function () {
    ...
    let tokenURI = await dreamfactoryNFT.tokenURI(0);
  ...
  it("Should fail if more than 2 tokens are attempted to be minted", async function () {
    ...
    await expect(dreamfactoryNFT.connect(owner).mint()).to.be.revertedWith(
