...
class Config:
    ...
    def __init__(self):
        ...
        self.DISABLE_PROVIDER_CONFIG_VALIDATION = get_bool_env(
            'DISABLE_PROVIDER_CONFIG_VALIDATION')

 
        # JWT shared with dapp
        self.JWT_SHARED_SECRET_KEY = get_env('JWT_SHARED_SECRET_KEY')

        # Kafka to communicate with external service
        self.EXTERNAL_KAFKA_ENABLE = get_bool_env(
            'EXTERNAL_KAFKA_ENABLE')
        self.EXTERNAL_KAFKA_BROKERS = get_env('EXTERNAL_KAFKA_BROKERS')
        self.EXTERNAL_KAFKA_CLIENT_ID = get_env('EXTERNAL_KAFKA_CLIENT_ID')

        # Redis to communicate with external service, to check quota
        self.EXTERNAL_QUOTA_REDIS_ENABLE = get_bool_env(
            'EXTERNAL_QUOTA_REDIS_ENABLE')
        self.EXTERNAL_QUOTA_REDIS_HOST = get_env('EXTERNAL_QUOTA_REDIS_HOST')
        self.EXTERNAL_QUOTA_REDIS_PORT = get_env('EXTERNAL_QUOTA_REDIS_PORT')
        self.EXTERNAL_QUOTA_REDIS_PASSWORD = get_env(
            'EXTERNAL_QUOTA_REDIS_PASSWORD')
        self.EXTERNAL_QUOTA_REDIS_DB = get_env('EXTERNAL_QUOTA_REDIS_DB')
        self.EXTERNAL_QUOTA_REDIS_USERNAME = get_env(
            'EXTERNAL_QUOTA_REDIS_USERNAME')
        self.EXTERNAL_QUOTA_REDIS_USE_SSL = get_bool_env(
            'EXTERNAL_QUOTA_REDIS_USE_SSL')
        
        self.ENABLE_SW = get_bool_env(
            'ENABLE_SW')
