...
import { chatAppStart, fetchAppInfo, fetchAppParams, fetchChatList, fetchConversations, fetchSuggestedQuestions, sendChatMessage, updateFeedback } from '@/service/share'
import type { ConversationItem, SiteInfo, Web3UserInfo } from '@/models/share'
...
const Main: FC<IMainProps> = ({
  ...
  */
  ...
  const [web3UserInfo, setWeb3UserInfo] = useState<Web3UserInfo | null | undefined>()
  ...
  useEffect(() => {
    if (siteInfo?.title) {
      if (plan !== 'basic')
        ...
        document.title = `${siteInfo.title} - Powered by Dynamo`
    }
  }, [siteInfo?.title, plan])
  ...
  */
  ...
  const fetchInitData = () => {
    chatAppStart(isInstalledApp, installedAppInfo?.id) // async call for current user to notify external services by kafka message
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const { app_id: appId, site: siteInfo, plan, external_user_id: wallet_address }: any = appData
        setWeb3UserInfo({ wallet_address })
