...
const baseFetch = (
  ...
  if (isPublicAPI) {
    ...
    options.headers.set('Authorization', `bearer ${sharedToken} ${user_token}`)
    options.headers.set('x-public-api', 'public-api')
  }
            ...
            switch (res.status) {
              case 401: {
                ...
                if (IS_CE_EDITION) {
                  ...
                  return Promise.reject(new Error('IS_CE_EDITION'))
                }
                globalThis.location.href = loginUrl
                break
              }
