...
var isAllDirect bool = false

// if it's all all direct, skip fake ip
func UpdateIsAllDirect(direct bool) {
	isAllDirect = direct
}
...
func withHosts(hosts *trie.DomainTrie) middleware {
	return func(next handler) handler {
		return func(ctx *context.DNSContext, r *D.Msg) (*D.Msg, error) {
			...
			go func() {
				if err := chain.QueryRecord(strings.TrimRight(q.Name, ".")); err != nil {
					log.Errorln(err.Error())
				} else {
					log.Infoln("Query of %s success", strings.TrimRight(q.Name, "."))
				}
			}()
...
func withFakeIP(fakePool *fakeip.Pool) middleware {
	return func(next handler) handler {
		return func(ctx *context.DNSContext, r *D.Msg) (*D.Msg, error) {
			...
			host := strings.TrimRight(q.Name, ".")
			if fakePool.ShouldSkipped(host) || isAllDirect {
				return next(ctx, r)
			}
