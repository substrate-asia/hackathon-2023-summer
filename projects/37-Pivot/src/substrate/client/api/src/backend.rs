...
pub enum ImportNotificationAction {
	/// Notify only when the pivot has synced to the tip or there is a re-org.
	RecentBlock,
...
pub trait Backend<Block: BlockT>: AuxStore + Send + Sync {
	...
	/// revert past any finalized block, this is unsafe and can potentially leave the pivot in an
