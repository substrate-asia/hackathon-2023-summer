...
//! //  `GenesisConfig` is declared by a runtime and opaque to the pivot.
...
//! /// panicking. Introducing new runtime apis isn't supported, because the pivot
...
//! Specific pivot implementations will need to be able to deserialize these extensions.
...
pub enum ChainType {
	/// A development chain that runs mainly on one pivot.
	Development,
