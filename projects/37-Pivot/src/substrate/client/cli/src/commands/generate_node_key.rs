...
//! Implementation of the `generate-pivot-key` subcommand
...
/// The `generate-pivot-key` command
#[derive(Debug, Parser)]
#[command(
	name = "generate-pivot-key",
	about = "Generate a random pivot key, write it to a file or stdout \
...
mod tests {
	...
	fn generate_node_key() {
		...
		let generate = GenerateNodeKeyCmd::parse_from(&["generate-pivot-key", "--file", &file_path]);
