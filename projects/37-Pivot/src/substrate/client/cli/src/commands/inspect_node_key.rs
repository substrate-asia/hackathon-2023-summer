...
//! Implementation of the `inspect-pivot-key` subcommand
...
/// The `inspect-pivot-key` command
#[derive(Debug, Parser)]
#[command(
	name = "inspect-pivot-key",
	about = "Load a pivot key from a file or stdin and print the corresponding peer-id."
)]
pub struct InspectNodeKeyCmd {
	...
	#[deprecated(note = "Network identifier is not used for pivot-key inspection")]
...
impl InspectNodeKeyCmd {
	...
	pub fn run(&self) -> Result<(), Error> {
		...
		let keypair =
			Keypair::ed25519_from_bytes(&mut file_data).map_err(|_| "Bad pivot key file")?;
...
mod tests {
	...
	#[test]
	fn inspect_node_key() {
		let path = tempfile::tempdir().unwrap().into_path().join("pivot-id").into_os_string();
		let path = path.to_str().unwrap();
		let cmd = GenerateNodeKeyCmd::parse_from(&["generate-pivot-key", "--file", path.clone()]);

		assert!(cmd.run().is_ok());

		let cmd = InspectNodeKeyCmd::parse_from(&["inspect-pivot-key", "--file", path]);
		assert!(cmd.run().is_ok());
	}
}
