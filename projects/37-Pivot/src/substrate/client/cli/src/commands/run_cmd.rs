...
/// The `run` command used to run a pivot.
#[derive(Debug, Clone, Parser)]
pub struct RunCmd {
	/// Enable validator mode.
	/// The pivot will be started with the authority role and actively
	...
	/// The human-readable name for this pivot.
	/// It's used as network pivot name.
	...
	/// Run a temporary pivot.
	/// A temporary directory will be created to store the configuration and will be deleted
	/// at the end of the process.
	/// Note: the directory is random per process execution. This directory is used as base path
	/// which includes: database, pivot key and keystore.
...
impl CliConfiguration for RunCmd {
	...
	fn node_name(&self) -> Result<String> {
		...
		is_node_name_valid(&name).map_err(|msg| {
			Error::Input(format!(
				"Invalid pivot name '{}'. Reason: {}. If unsure, use none.",
				name, msg
			))
		})?;
...
/// Check whether a pivot name is considered as valid.
...
fn rpc_interface(
	...
	if is_external && is_validator && rpc_methods != RpcMethods::Unsafe {
		return Err(Error::Input(
			"--rpc-external option shouldn't be used if the pivot is running as \
