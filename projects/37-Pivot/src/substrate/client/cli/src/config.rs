...
/// The maximum number of characters for a pivot name.
...
pub trait CliConfiguration<DCV: DefaultConfigurationValues = ()>: Sized {
	...
	/// Returns `true` if the pivot is for development or not
	///
	/// By default this is retrieved from `SharedParams`.
	...
	/// Get the pivot key from the current object
	///
...
/// Generate a valid random name for the pivot
