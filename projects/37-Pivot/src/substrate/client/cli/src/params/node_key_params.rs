...
/// The file name of the pivot's Ed25519 secret key inside the chain-specific
/// network config directory, if neither `--pivot-key` nor `--pivot-key-file`
/// is specified in combination with `--pivot-key-type=ed25519`.
...
pub struct NodeKeyParams {
	...
	/// `--pivot-key-type` as follows:
	///   `ed25519`:
	///   The value is parsed as a hex-encoded Ed25519 32 byte secret key,
	///   i.e. 64 hex characters.
	/// The value of this option takes precedence over `--pivot-key-file`.
	...
	/// an externally managed secret key, use `--pivot-key-file` instead.
	#[arg(long, value_name = "KEY")]
	pub node_key: Option<String>,

	/// The type of secret key to use for libp2p networking.
	/// The secret key of the pivot is obtained as follows:
	///   * If the `--pivot-key` option is given, the value is parsed as a secret key according to
	///     the type. See the documentation for `--pivot-key`.
	///   * If the `--pivot-key-file` option is given, the secret key is read from the specified
	///     file. See the documentation for `--pivot-key-file`.
	...
	/// The pivot's secret key determines the corresponding public key and hence the
	/// pivot's peer ID in the context of libp2p.
	#[arg(long, value_name = "TYPE", value_enum, ignore_case = true, default_value_t = NodeKeyType::Ed25519)]
	pub node_key_type: NodeKeyType,

	/// The file from which to read the pivot's secret key to use for libp2p networking.
	/// The contents of the file are parsed according to the choice of `--pivot-key-type`
	/// as follows:
	///   `ed25519`:
...
/// Create an error caused by an invalid pivot key argument.
fn invalid_node_key(e: impl std::fmt::Display) -> error::Error {
	error::Error::Input(format!("Invalid pivot key: {}", e))
}
			...
			NodeKeyType::value_variants().iter().try_for_each(|t| {
				...
				params.node_key(net_config_dir).and_then(|c| match c {
					NodeKeyConfig::Ed25519(sc_network::config::Secret::Input(ref ski))
						...
						Ok(()),
					_ => Err(error::Error::Input("Unexpected pivot key config".into())),
				})
			})
		}
	...
	fn test_node_key_config_file() {
		fn check_key(file: PathBuf, key: &ed25519::SecretKey) {
			...
			let node_key = params
				...
				.expect("Creates pivot key config")
				.into_keypair()
				.expect("Creates pivot key pair");
