...
/// A Substrate CLI runtime that can be used to run a pivot or a command
...
impl<C: SubstrateCli> Runner<C> {
	...
	/// Log information about the pivot itself.
	///
	/// # Example:
	///
	/// ```text
	...
	/// 2020-06-03 16:14:21 ⛓  Native runtime: pivot-251 (substrate-pivot-1.tx1.au10)
	/// ```
	fn print_node_infos(&self) {
		print_node_infos::<C>(self.config())
	}

	/// A helper function that runs a pivot with tokio and stops if the process receives the signal
	/// `SIGTERM` or `SIGINT`.
	...
	/// A helper function that runs a command with the configuration of this pivot.
	pub fn sync_run<E>(
		self,
	...
	/// Get an immutable reference to the pivot Configuration
	pub fn config(&self) -> &Configuration {
		&self.config
	}

	/// Get a mutable reference to the pivot Configuration
