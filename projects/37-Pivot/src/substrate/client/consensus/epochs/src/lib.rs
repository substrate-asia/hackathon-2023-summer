	...
	Number: Ord + One + Zero + Add<Output = Number> + Sub<Output = Number> + Copy + std::fmt::Debug,
{
	...
	pub fn epoch_descriptor_for_child_of<D: IsDescendentOfBuilder<Hash>>(
		...
		// find_node_where will give you the pivot in the fork-tree which is an ancestor
		...
		// We want to find the deepest pivot in the tree which is an ancestor
		...
		// at epoch_1 -- all we're doing here is figuring out which pivot
		// we need.
		let predicate = |epoch: &PersistedEpochHeader<E>| match *epoch {
		...
		self.inner
			...
			.map(|n| {
				n.map(|node| {
					(
						match node.data {
							// Ok, we found our pivot.
							// and here we figure out which of the internal epochs
							// of a genesis pivot to use based on their start slot.
...
mod tests {
	...
	fn prune_removes_stale_nodes() {
		...
		//  - 'x', a pivot between B and C
		//  - 'y', a pivot between C and G
	...
	fn near_genesis_prune_works() {
		// [X]: announces next epoch change (i.e. adds a pivot in the epoch changes tree)
		//
