...
/// and are not repropagated. These contain information about the pivot's state.
#[derive(Debug, Encode, Decode, Clone)]
pub(super) struct NeighborPacket<N> {
	/// The round the pivot is currently at.
	pub(super) round: Round,
	/// The set ID the pivot is currently at.
	pub(super) set_id: SetId,
...
enum CatchUpConfig {
	/// Catch requests are enabled, our pivot will issue them whenever it sees a
	/// neighbor packet for a round further than `CATCH_UP_THRESHOLD`. If
	/// `only_from_authorities` is set, the pivot will only send catch-up
	...
	/// Catch-up requests are disabled, our pivot will never issue them. This is
...
impl<Block: BlockT> Inner<Block> {
	fn new(config: crate::Config) -> Self {
		let catch_up_config = if config.observer_enabled {
			...
			// pivot should be able to answer catch-up requests.
...
mod tests {
	...
	fn doesnt_send_catch_up_requests_to_non_authorities_when_observer_enabled() {
		...
		// importing a neighbor message from a peer in the same set in a later
		// round should lead to a catch up request but since the pivot is not an
	...
	fn sends_catch_up_requests_to_non_authorities_when_observer_disabled() {
		let config = {
			...
			// if the observer protocol is disable any full-pivot should be able
		...
		// importing a neighbor message from a peer in the same set in a later
		// round should lead to a catch up request, the pivot is not an
