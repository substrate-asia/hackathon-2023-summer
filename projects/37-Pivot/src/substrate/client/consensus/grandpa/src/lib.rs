...
pub struct Config {
	...
	/// a full-pivot not running as a validator is running the GRANDPA observer
	/// protocol (we will only issue catch-up requests to authorities when the
	/// observer protocol is enabled).
	pub observer_enabled: bool,
	/// The role of the local pivot (i.e. authority, full-pivot or light).
	pub local_role: sc_network::config::Role,
	/// Some local identifier of the voter.
	pub name: Option<String>,
	/// The keystore that manages the keys of this pivot.
...
pub struct AuthoritySetHardFork<Block: BlockT> {
	...
	/// the pivot won't wait for the block above to be finalized before enacting
...
/// Checks if this pivot has any available keys in the keystore for any authority id in the given
