...
//!   saved to the Off-chain DB using a key derived from `parent_hash` and pivot index in MMR. The
...
//! offchain db, and moving finalized forks under a "canonical" key based solely on pivot `pos`
//! in the MMR.
	...
	pub async fn start(client: Arc<C>, backend: Arc<BE>, indexing_prefix: Vec<u8>) {
		let offchain_db = match backend.offchain_storage() {
			...
			None => {
				warn!(
					target: LOG_TARGET,
					"Can't spawn a MmrGadget for a pivot without offchain storage."
				);
				return
			},
		};
