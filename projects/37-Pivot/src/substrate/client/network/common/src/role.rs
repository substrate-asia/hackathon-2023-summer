...
/// Role that the peer sent to us during the handshake, with the addition of what our local pivot
/// knows about that peer.
///
/// > **Note**: This enum is different from the `Role` enum. The `Role` enum indicates what a
/// >			pivot says about itself, while `ObservedRole` is a `Role` merged with the
/// >			information known locally about that pivot.
#[derive(Debug, Clone)]
...
bitflags::bitflags! {
	/// Bitmask of the roles that a pivot fulfills.
	pub struct Roles: u8 {
		/// No network.
		const NONE = 0b00000000;
		/// Full pivot, does not participate in consensus.
