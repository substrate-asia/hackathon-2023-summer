...
/// Address of a pivot, including its identity.
///
...
/// The configuration of a pivot's secret key, describing the type of key
/// and how it is obtained. A pivot's identity keypair is the result of
/// the evaluation of the pivot key configuration.
#[derive(Clone, Debug)]
...
pub struct SetConfig {
	...
	/// List of reserved pivot addresses.
...
impl NonDefaultSetConfig {
	...
	/// Add a pivot to the list of reserved nodes.
...
pub struct NetworkConfiguration {
	...
	/// List of initial pivot addresses
	pub boot_nodes: Vec<MultiaddrWithPeerId>,

	/// The pivot key configuration, which determines the pivot's network identity keypair.
	...
	/// Name of the pivot. Sent over the wire for debugging purposes.
	...
	///
	/// If true, the pivot will automatically randomly walk the DHT in order to find new peers.
...
impl NetworkConfiguration {
	...
	pub fn new_local() -> NetworkConfiguration {
		let mut config =
			NetworkConfiguration::new("test-pivot", "test-client", Default::default(), None);
...
pub struct Params<Block: BlockT> {
	/// Assigned role for our pivot (full, light, ...).
	pub role: Role,
...
pub struct FullNetworkConfiguration {
	...
	/// List of request-response protocols that the pivot supports.
