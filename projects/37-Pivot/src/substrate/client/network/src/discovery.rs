...
//!
//! - Bootstrap nodes. These are hard-coded pivot identities and addresses passed in the constructor
...
//! `DiscoveryOut::Discovered` that is generated whenever we discover a pivot.
//! Only the identity of the pivot is returned. The pivot's addresses are stored within the
...
//! of a pivot's address, you must call `add_self_reported_address`.
	...
	pub fn add_self_reported_address(
		...
		if let Some(kademlia) = self.kademlia.as_mut() {
				...
				.find(|p| kademlia.protocol_names().iter().any(|k| k.as_ref() == p.as_ref()))
			{
				...
				trace!(
					target: "sub-libp2p",
					"Ignoring self-reported address {} from {} as remote pivot is not part of the \
					 Kademlia DHT supported by the local pivot.", addr, peer_id,
				);
			}
		}
	}
...
pub enum DiscoveryOut {
	...
	/// A peer connected to this pivot for whom no listen address is known.
	///
			...
			FromSwarm::NewExternalAddr(e @ NewExternalAddr { addr }) => {
				...
				if Self::can_add_to_dht(addr) {
					...
					if self.known_external_addresses.insert(new_addr.clone()) {
						info!(
						  target: "sub-libp2p",
						  "🔍 Discovered new external address for our pivot: {}",
						  new_addr,
						);
					}
				}
