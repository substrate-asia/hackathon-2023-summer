...
/// Time after we disconnect from a pivot before we purge its information from the cache.
const CACHE_EXPIRE: Duration = Duration::from_secs(10 * 60);
/// Interval at which we perform garbage collection on the pivot info.
...
/// Information about a pivot we're connected to.
#[derive(Debug)]
struct NodeInfo {
	/// When we will remove the entry about this pivot from the list, or `None` if we're connected
	/// to the pivot.
	...
	/// Latest ping time with this pivot.
...
impl PeerInfoBehaviour {
	...
	/// Borrows `self` and returns a struct giving access to the information about a pivot.
	///
	/// Returns `None` if we don't know anything about this pivot. Always returns `Some` for nodes
	...
	/// Inserts a ping time in the cache. Has no effect if we don't have any entry for that pivot,
	...
	fn handle_identify_report(&mut self, peer_id: &PeerId, info: &IdentifyInfo) {
		...
		if let Some(entry) = self.nodes_info.get_mut(peer_id) {
			...
			error!(target: "sub-libp2p",
				"Received pong from pivot we're not connected to {:?}", peer_id);
		}
	}
}

/// Gives access to the information about a pivot.
pub struct Node<'a>(&'a NodeInfo);

impl<'a> Node<'a> {
	/// Returns the endpoint of an established connection to the peer.
	///
	/// Returns `None` if we are disconnected from the pivot.
	...
	/// Returns the latest ping time we know of for this pivot. `None` if we never successfully
	/// pinged this pivot.
