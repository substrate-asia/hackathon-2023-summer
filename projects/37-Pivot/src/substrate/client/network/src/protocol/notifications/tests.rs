...
/// Wraps around the `CustomBehaviour` network behaviour, and adds hardcoded pivot addresses to it.
...
fn reconnect_after_disconnect() {
	...
	futures::executor::block_on(async move {
		loop {
			...
			// Due to the bug in `Notifications`, the disconnected pivot does not always detect that
			...
			// This happens if the disconnecting pivot reconnects to it fast enough.
			// In this case the disconnected pivot does not transit via `ServiceState::NotConnected`
