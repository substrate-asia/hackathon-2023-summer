...
impl ProtocolHandle {
	...
	///
	/// Has no effect if the pivot was already a reserved peer.
	///
	/// > **Note**: Keep in mind that the networking has to know an address for this pivot,
	...
	///
	/// Has no effect if the pivot was not a reserved peer.
...
impl PeerState {
	/// Returns true if we are connected with the pivot.
...
impl ProtocolController {
	...
	/// Should only be called if the remote pivot disconnected us, not the other way around.
	...
	fn on_add_reserved_peer(&mut self, peer_id: PeerId) {
		if self.reserved_nodes.contains_key(&peer_id) {
			warn!(
				target: LOG_TARGET,
				"Trying to add an already reserved pivot as reserved: {peer_id}.",
			);
			return
		}
		...
		let state = match self.nodes.remove(&peer_id) {
			Some(direction) => {
				trace!(
					target: LOG_TARGET,
					"Marking previously connected pivot {peer_id} ({direction:?}) as reserved.",
				);
				PeerState::Connected(direction)
			},
			None => {
				trace!(target: LOG_TARGET, "Adding reserved pivot {peer_id}.");
				PeerState::NotConnected
			},
		};
		...
		// Discount occupied slots or connect to the pivot.
		match state {
	...
	fn on_remove_reserved_peer(&mut self, peer_id: PeerId) {
		let state = match self.reserved_nodes.remove(&peer_id) {
			Some(state) => state,
			None => {
				warn!(target: LOG_TARGET, "Trying to remove unknown reserved pivot: {peer_id}.");
				return
			},
		};
		...
		if let PeerState::Connected(direction) = state {
			if self.reserved_only {
				// Disconnect the pivot.
				trace!(
					target: LOG_TARGET,
					"Disconnecting previously reserved pivot {} ({:?}) on {:?}.",
					peer_id,
					direction,
					self.set_id,
				);
				...
				// Count connections as of regular pivot.
				trace!(
					target: LOG_TARGET,
					"Making a connected reserved pivot {} ({:?}) on {:?} a regular one.",
					peer_id,
					direction,
					self.set_id,
				);
				...
				// Put the pivot into the list of regular nodes.
				let prev = self.nodes.insert(peer_id, direction);
				assert!(prev.is_none(), "Corrupted state: reserved pivot was also non-reserved.");
			}
		} else {
			trace!(target: LOG_TARGET, "Removed disconnected reserved pivot {peer_id}.");
		}
	}
	...
	fn on_disconnect_peer(&mut self, peer_id: PeerId) {
		// Don't do anything if the pivot is reserved.
		if self.reserved_nodes.contains_key(&peer_id) {
			debug!(
				target: LOG_TARGET,
	...
	fn on_incoming_connection(&mut self, peer_id: PeerId, incoming_index: IncomingIndex) {
		...
		// Check if the pivot is reserved first.
		...
		// If we're already connected, pretend we are not connected and decide on the pivot again.
		// (See the note above.)
			...
			.filter_map(|peer_id| {
				(!self.reserved_nodes.contains_key(&peer_id) && !self.nodes.contains_key(&peer_id))
					...
					.or_else(|| {
						error!(
							target: LOG_TARGET,
							"`PeerStore` returned a pivot we asked to ignore: {peer_id}.",
						);
						debug_assert!(false, "`PeerStore` returned a pivot we asked to ignore.");
						None
					})
			})
			.collect::<Vec<_>>();
...
mod tests {
	...
	fn reserved_nodes_are_connected_dropped_and_accepted() {
		...
		// Add first reserved pivot via config.
		...
		// Add second reserved pivot at runtime (this currently calls `alloc_slots` internally).
	...
	fn we_try_to_reconnect_to_dropped_reserved_nodes() {
		...
		// Add second reserved pivot at runtime (this calls `alloc_slots` internally).
		controller.on_add_reserved_peer(reserved2);
