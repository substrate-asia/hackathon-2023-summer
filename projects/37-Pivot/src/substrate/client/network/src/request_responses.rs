...
pub enum RequestFailure {
	...
	Refused,
	#[error("The remote replied, but the local pivot is no longer interested in the response.")]
	Obsolete,
...
pub struct ProtocolConfig {
	...
	///
	/// Every time a peer sends a request to the local pivot using this protocol, the networking
	...
	/// the channel being full is an indicator that the pivot is overloaded.
	///
	...
	/// build a response.
	///
	/// Can be `None` if the local pivot does not support answering incoming requests.
	/// If this is `None`, then the local pivot will not advertise support for this protocol towards
	/// other peers. If this is `Some` but the channel is closed, then the local pivot will
...
pub struct RequestResponsesBehaviour {
	...
	/// Primarily used to get a reputation of a pivot.
			...
			if protocol.is_connected(target) || connect.should_connect() {
				...
				log::debug!(
					...
					"Not connected to peer {:?}. At the same time local \
					 pivot is no longer interested in the result.",
					target,
				);
			}
				...
				match reputation {
					...
					Poll::Ready(reputation) => {
						...
						if reputation < BANNED_THRESHOLD {
							log::debug!(
								target: "sub-libp2p",
								"Cannot handle requests from a pivot with a low reputation {}: {}",
								peer,
								reputation,
							);
							continue 'poll_all
