...
pub struct NetworkService<B: BlockT + 'static, H: ExHashT> {
	...
	/// Local copy of the `PeerId` of the local pivot.
	local_peer_id: PeerId,
	/// The `KeyPair` that defines the `PeerId` of the local pivot.
		...
		network_config.default_peers_set.reserved_nodes = network_config
			...
			.filter(|reserved_node| {
				if reserved_node.peer_id == local_peer_id {
					warn!(
						target: "sub-libp2p",
						"Local peer ID used in reserved pivot, ignoring: {}",
						reserved_node,
					);
					false
				} else {
