...
mod rep {
	...
	///
	/// This forces pivot to verify it, thus the negative value here. Once statement is verified,
	...
	S: SyncEventStream + sp_consensus::SyncOracle,
{
		...
		match event {
			...
			Event::NotificationsReceived { remote, messages } => {
				for (protocol, message) in messages {
					...
					// Accept statements only when pivot is not major syncing
	...
	fn propagate_statements(&mut self) {
		// Send out statements only when pivot is not major syncing
		if self.sync.is_major_syncing() {
			return
		}
