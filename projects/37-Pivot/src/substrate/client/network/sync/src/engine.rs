...
/// If the block announces stream to peer has been inactive for 30 seconds meaning local pivot
/// has not sent or received block announcements to/from the peer, report the pivot for inactivity,
		...
		prometheus_endpoint::register(
			SourcedGauge::new(
				&Opts::new(
					...
					"Whether the pivot is performing a major sync or not.",
				),
...
pub struct SyncingEngine<B: BlockT, Client> {
	/// State machine that handles the list of in-progress requests. Only full pivot peers are
	/// registered.
	...
	/// All connected peers. Contains both full and light pivot peers.
		...
		+ 'static,
{
	...
	pub fn poll(&mut self, cx: &mut std::task::Context) -> Poll<()> {
		...
		while let Poll::Ready(()) = self.tick_timeout.poll_unpin(cx) {
			...
			// be replaced with some other pivot that is willing to send us block announcements.
