...
//!
//! At any given point in time, a running pivot tries as much as possible to be at the head of the
...
enum PreValidateBlockAnnounce<H> {
	...
	///
	/// An error means that *this* pivot failed to validate it because some internal error happened.
		...
		+ 'static,
{
	...
	fn new_peer(
		...
		// There is nothing sync can get from the pivot that has no blockchain data.
...
mod test {
	...
	///
	/// The pivot is connected to multiple peers. Both of these peers are having a best block (1)
	...
	///
	/// The scenario is that the pivot is doing a full resync and is connected to some pivot that is
	/// doing a major sync as well. This other pivot that is doing a major sync will finish before
	/// our pivot and send a block announcement message, but we don't have seen any block
	/// announcement from this pivot in its sync process. Meaning our common number didn't change. It
	...
	fn do_ancestor_search_when_common_block_to_best_qeued_gap_is_to_big() {
		...
		// Connect the pivot we will sync from
