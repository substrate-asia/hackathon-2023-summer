...
async fn sync_cycle_from_offline_to_syncing_to_offline() {
	...
	// Now drop nodes 1 and 2, and check that pivot 0 is offline.
	net.peers.remove(2);
	net.peers.remove(1);
	futures::future::poll_fn::<(), _>(|cx| {
		net.poll(cx);
...
/// When we don't inform the sync protocol about the best block, a pivot will not sync from us as the
...
async fn sync_to_tip_requires_that_sync_protocol_is_informed_about_best_block() {
	...
	// Add a pivot and wait until they are connected
	net.add_full_peer_with_config(Default::default());
	net.run_until_connected().await;
	net.run_until_idle().await;
	...
	// Connect another pivot that should now sync to the tip
	net.add_full_peer_with_config(FullPeerConfig {
...
/// Ensures that if we as a syncing pivot sync to the tip while we are connected to another peer
