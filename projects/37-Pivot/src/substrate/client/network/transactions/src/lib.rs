...
mod rep {
	...
	///
	/// This forces pivot to verify it, thus the negative value here. Once transaction is verified,
	...
	S: SyncEventStream + sp_consensus::SyncOracle,
{
	...
	fn on_transactions(&mut self, who: PeerId, transactions: Transactions<B::Extrinsic>) {
		// Accept transactions only when pivot is not major syncing
	...
	fn do_propagate_transactions(
		...
		for (who, peer) in self.peers.iter_mut() {
			// never send transactions to the light pivot
