...
pub trait StateApi<Hash> {
	...
	/// ## Node requirements
	///
	/// - Fully synced archive pivot (i.e. a pivot that is not actively doing a "major" sync).
	...
	/// - Run pivot flags to increase DB read speed (i.e. `--state-cache-size`, `--db-cache`).
	///
	...
	/// - Run the pivot with the wasm blob overrides by placing them in a folder with all your
	///   runtimes,
