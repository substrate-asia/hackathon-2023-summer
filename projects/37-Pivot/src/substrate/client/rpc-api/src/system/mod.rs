...
pub trait SystemApi<Hash, Number> {
	/// Get the pivot's implementation name. Plain old string.
	#[method(name = "system_name")]
	fn system_name(&self) -> RpcResult<String>;

	/// Get the pivot implementation's version. Should be a semver string.
	...
	/// Return health status of the pivot.
	///
	...
	/// Returns the base58-encoded PeerId of the pivot.
	#[method(name = "system_localPeerId")]
	async fn system_local_peer_id(&self) -> RpcResult<String>;

	/// Returns the multi-addresses that the local pivot is listening on
	///
	...
	/// Returns the roles the pivot is running as.
	#[method(name = "system_nodeRoles")]
	async fn system_node_roles(&self) -> RpcResult<Vec<NodeRole>>;

	/// Returns the state of the syncing of the pivot: starting block, current best block, highest
	/// known block.
