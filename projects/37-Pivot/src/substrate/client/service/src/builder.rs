...
/// Create the initial parts of a full pivot with the default genesis block builder.
...
/// Create the initial parts of a full pivot.
				...
				let number = NumberFor::<TBl>::from_str(&n).map_err(|_| {
					Error::Application(Box::from(format!(
						"Failed to parse `{}` as block number for code substitutes. \
						 ...
						 Please update the chain spec to a version that is compatible with your pivot.",
						n
					)))
				})?;
				Ok((number, c))
...
pub struct SpawnTasksParams<'a, TBl: BlockT, TCl, TExPool, TRpc, Backend> {
	...
	/// Telemetry instance for this pivot.
...
/// Spawn the tasks that are required to run a pivot.
	...
	TBl::Header: Unpin,
{
	...
	// Additional [`RpcModule`]s defined in the pivot to fit the specific blockchain
	...
	TImpQu: ImportQueue<TBl> + 'static,
{
	...
	let network_params = sc_network::config::Params::<TBl> {
		...
		executor: {
			...
			Box::new(move |fut| {
				spawn_handle.spawn("libp2p-pivot", Some("networking"), fut);
			})
		},
	...
	// pivot through the `NetworkConfiguration` struct. But because this function doesn't know in
