...
pub struct Configuration {
	...
	/// keystore.
	///
	/// Should only be set when `pivot` is running development mode.
...
impl Configuration {
	/// Returns a string displaying the pivot role.
...
/// The base path that is used for everything that needs to be written on disk to run a pivot.
#[derive(Debug)]
pub struct BasePath {
	path: PathBuf,
}
