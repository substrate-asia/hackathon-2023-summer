...
>(
	...
	let root = root.path().join(format!("pivot-{}", index));
		...
		self.runtime.block_on(async {
			...
			for (key, authority) in authorities {
				...
				let (service, user_data) =
					authority(node_config).expect("Error creating test pivot service");
			...
			for full in full {
				...
				let addr = node_config.network.listen_addresses.first().unwrap().clone();
				let (service, user_data) =
					full(node_config).expect("Error creating test pivot service");
