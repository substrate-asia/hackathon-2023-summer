...
//! overlay allows to get any trie pivot that was inserted in any of the blocks within the window.
...
/// Basic set of requirements for the Block hash and pivot key types.
pub trait Hash:
	Send
	+ Sync
	+ Sized
...
impl<BlockHash: Hash, Key: Hash, D: MetaDb> StateDbSync<BlockHash, Key, D> {
	...
	fn canonicalize_block(&mut self, hash: &BlockHash) -> Result<CommitSet<Key>, Error<D::Error>> {
		...
		// the database atomically to keep their consistency when restarting the pivot
