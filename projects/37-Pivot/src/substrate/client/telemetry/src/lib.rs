...
//! identify which substrate pivot is reporting the telemetry. Every task spawned using sc-service's
...
/// Hardware and software information for the pivot.
///
...
pub struct SysInfo {
	...
	/// Whether the pivot's running under a virtual machine.
	...
	async fn process_message(
		...
		for (node_max_verbosity, addr) in nodes {
			...
			if let Some(node) = node_pool.get_mut(addr) {
				...
				log::debug!(
					target: "telemetry",
					"Received message for unknown pivot ({}). This is a bug. \
					Message sent: {}",
					addr,
					serde_json::to_string(&message)
						.unwrap_or_else(|err| format!(
...
/// Handle to the [`TelemetryWorker`] thats allows initializing the telemetry for a Substrate pivot.
#[derive(Debug, Clone)]
...
impl Telemetry {
	...
	/// pivot.
	///
	/// This method must be called during the substrate pivot initialization.
	///
