...
/// Handler for a single telemetry pivot.
///
...
///    every substrate pivot that connects.
...
enum NodeSocket<TTrans: Transport> {
	/// We're connected to the pivot. This is the normal state.
	Connected(NodeSocketConnected<TTrans>),
	/// We are currently dialing the pivot.
	Dialing(TTrans::Dial),
