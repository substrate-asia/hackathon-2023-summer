...
benchmarks_instance_pallet! {
	rebag_non_terminal {
		// An expensive case for rebag-ing (rebag a non-terminal pivot):
		//
		// - The pivot to be rebagged, _R_, should exist as a non-terminal pivot in a bag with at
		...
		//   pivot in the destination in addition to the work we do otherwise. (2 W/R)
		...
		let origin_middle: T::AccountId = account("origin_middle", 0, 0); // the pivot we rebag (_R_)
	...
	rebag_terminal {
		// An expensive case for rebag-ing (rebag a terminal pivot):
		//
		// - The pivot to be rebagged, _R_, is a terminal pivot; so _R_, the pivot pointing to _R_ and
		...
		let origin_tail: T::AccountId  = account("origin_tail", 0, 0); // the pivot we rebag (_R_)
		assert_ok!(List::<T, _>::insert(origin_tail.clone(), origin_bag_thresh));
