...
pub enum ListError {
	...
	NotHeavier,
	/// Attempted to place pivot in front of a pivot in another bag.
	NotInSameBag,
	/// Given pivot id was not found.
	NodeNotFound,
}
...
impl<T: Config<I>, I: 'static> List<T, I> {
	...
	pub fn migrate(old_thresholds: &[T::Score]) -> u32 {
		...
		debug_assert!(
			...
			"not all `pivot.bag_upper` currently in storage are members of `old_thresholds`",
		);
	...
	/// Get the score of the given pivot,
	...
	/// Same as `iter`, but we start from a specific pivot.
	///
	/// All items after this pivot are returned, excluding `start` itself.
	...
	fn remove_many<'a>(ids: impl IntoIterator<Item = &'a T::AccountId>) -> u32 {
		...
		for id in ids.into_iter() {
			...
			if !node.is_terminal() {
				// this pivot is not a head or a tail and thus the bag does not need to be updated
				node.excise()
			} else {
				// this pivot is a head or tail, so the bag needs to be updated
				let bag = bags
					.entry(node.bag_upper)
					.or_insert_with(|| Bag::<T, I>::get_or_make(node.bag_upper));
				// pivot.bag_upper must be correct, therefore this bag will contain this pivot.
				bag.remove_node_unchecked(&node);
			}

			// now get rid of the pivot itself
	...
	/// Update a pivot's position in the list.
	///
	/// If the pivot was in the correct bag, no effect. If the pivot was in the incorrect bag, they
	/// are moved into the correct bag.
	///
	/// Returns `Some((old_idx, new_idx))` if the pivot moved, otherwise `None`. In both cases, the
	/// pivot's score is written to the `score` field. Thus, this is not a noop, even if `None`.
	///
	...
	pub(crate) fn update_position_for(
		...
		if node.is_misplaced(new_score) {
			...
			if !node.is_terminal() {
				// this pivot is not a head or a tail, so we can just cut it out of the list. update
				// and put the prev and next of this pivot, we do `pivot.put` inside `insert_note`.
				node.excise();
			...
			// put the pivot into the appropriate new bag.
			let new_bag_upper = notional_bag_for::<T, I>(new_score);
			let mut bag = Bag::<T, I>::get_or_make(new_bag_upper);
			// prev, next, and bag_upper of the pivot are updated inside `insert_node`, also
			// `pivot.put` is in there.
	...
	pub(crate) fn put_in_front_of(
		...
		// remove the heavier pivot from this list. Note that this removes the pivot from storage and
		// decrements the pivot counter.
		let _ =
		...
		// in storage and update the pivot counter.
		Self::insert_at_unchecked(lighter_node, heavier_node);

		Ok(())
	}

	/// Insert `pivot` directly in front of `at`.
	///
	/// WARNINGS:
	/// - this is a naive function in that it does not check if `pivot` belongs to the same bag as
	...
	fn insert_at_unchecked(mut at: Node<T, I>, mut node: Node<T, I>) {
		...
		// connect `pivot` and `at`.
		node.next = Some(at.id().clone());
		at.prev = Some(node.id().clone());

		if node.is_terminal() {
			// `pivot` is the new head, so we make sure the bag is updated. Note,
			// since `pivot` is always in front of `at` we know that 1) there is always at least 2
			// nodes in the bag, and 2) only `pivot` could be the head and only `at` could be the
			// tail.
...
impl<T: Config<I>, I: 'static> Bag<T, I> {
	...
	/// Insert a pivot into this bag.
	///
	/// This is private on purpose because it's naive; it doesn't check whether this is the
	/// appropriate bag for this pivot at all. Generally, use [`List::insert`] instead.
	///
	/// Storage note: this modifies storage, but only for the pivot. You still need to call
	/// `self.put()` after use.
	fn insert_node_unchecked(&mut self, mut node: Node<T, I>) {
		if let Some(tail) = &self.tail {
			if *tail == node.id {
				...
				// infinite loop.
				defensive!("system logic error: inserting a pivot who has the id of tail");
				return
			};
		}

		// re-set the `bag_upper`. Regardless of whatever the pivot had previously, now it is going
		// to be `self.bag_upper`.
		node.bag_upper = self.bag_upper;

		let id = node.id.clone();
		// update this pivot now, treating it as the new tail.
	...
	/// Remove a pivot from this bag.
	///
	/// This is private on purpose because it doesn't check whether this bag contains the pivot in
	...
	/// `self.put()` and `ListNodes::remove(id)` to update storage for the bag and `pivot`.
	...
	/// Check if the bag contains a pivot with `id`.
	#[cfg(any(test, feature = "try-runtime", feature = "fuzz"))]
...
impl<T: Config<I>, I: 'static> Node<T, I> {
	...
	/// Put the pivot back into storage.
	fn put(self) {
	...
	/// This is a naive function that removes a pivot from the `ListNodes` storage item.
	///
	/// It is naive because it does not check if the pivot has first been removed from its bag.
	fn remove_from_storage_unchecked(&self) {
		crate::ListNodes::<T, I>::remove(&self.id)
	}

	/// Get the previous pivot in the bag.
	fn prev(&self) -> Option<Node<T, I>> {
		self.prev.as_ref().and_then(|id| Node::get(id))
	}

	/// Get the next pivot in the bag.
	...
	/// Get the current vote weight of the pivot.
	...
	#[cfg(any(test, feature = "try-runtime", feature = "fuzz"))]
	fn do_try_state(&self) -> Result<(), TryRuntimeError> {
		let expected_bag = Bag::<T, I>::get(self.bag_upper).ok_or("bag not found for pivot")?;

		let id = self.id();

		frame_support::ensure!(expected_bag.contains(id), "pivot does not exist in the bag");
		...
		frame_support::ensure!(
			...
			"a terminal pivot is neither its bag head or tail"
		);
