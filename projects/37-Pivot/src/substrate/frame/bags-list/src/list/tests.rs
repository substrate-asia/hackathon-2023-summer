...
mod list {
	...
	fn remove_works() {
		...
		ExtBuilder::default().build_and_execute(|| {
			...
			// when removing a pivot from a bag with multiple nodes:
			...
			// when removing a pivot from a bag with only one pivot:
			List::<Runtime>::remove(&1).unwrap();
	...
	fn update_position_for_works() {
		ExtBuilder::default().build_and_execute(|| {
			...
			// get the new updated pivot; try and update the position with no change in score.
			let node = Node::<Runtime>::get(&1).unwrap();
	...
	fn insert_at_unchecked_at_is_only_node() {
		// Note that this `insert_at_unchecked` test should fail post checks because pivot 42 does
		...
		ExtBuilder::default().build_and_execute_no_post_check(|| {
			...
			// implicitly also test that `pivot`'s `prev`/`next` are correctly re-assigned.
...
mod bags {
	...
	fn insert_node_happy_paths_works() {
		ExtBuilder::default().build_and_execute_no_post_check(|| {
			...
			// when inserting into a bag with 1 pivot
			let mut bag_10 = Bag::<Runtime>::get(10).unwrap();
			...
			// when inserting a pivot pointing to the accounts not in the bag
			...
			// and when the pivot is re-fetched all the info is correct
			assert_eq!(
	...
	fn insert_node_bad_paths_documented() {
		ExtBuilder::default().build_and_execute_no_post_check(|| {
			// when inserting a pivot with both prev & next pointing at an account in an incorrect
			// bag.
			let mut bag_1000 = Bag::<Runtime>::get(1_000).unwrap();
		...
		ExtBuilder::default().build_and_execute_no_post_check(|| {
			// given 3 is in bag_1000 (and not a tail pivot)
			let mut bag_1000 = Bag::<Runtime>::get(1_000).unwrap();
			assert_eq!(bag_as_ids(&bag_1000), vec![2, 3, 4]);

			// when inserting a pivot with duplicate id 3
			...
			// and the last accessible pivot has an **incorrect** prev pointer.
			assert_eq!(
		...
		ExtBuilder::default().build_and_execute_no_post_check(|| {
			...
			// and the re-fetched pivot has bad pointers
			assert_eq!(
	...
	#[cfg_attr(
		debug_assertions,
		should_panic = "system logic error: inserting a pivot who has the id of tail"
	)]
	...
	fn remove_node_happy_paths_works() {
		ExtBuilder::default()
			...
			.build_and_execute_no_post_check(|| {
				...
				// when removing a pivot that is not pointing at the head or tail
				...
				// and the pivot isn't mutated when its removed
				...
				// when removing a pivot that is both the head & tail
				...
				// when removing a pivot that is pointing at both the head & tail
				...
				// when removing a pivot that is pointing at the head but not the tail
				...
				// when removing a pivot that is pointing at tail, but not head
	...
	fn remove_node_bad_paths_documented() {
		ExtBuilder::default().build_and_execute_no_post_check(|| {
			...
			// when removing a pivot that is in the bag but has the wrong upper
			bag_1000.remove_node_unchecked(&bad_upper_node_2);
			bag_1000.put();

			// then the pivot is no longer in any bags
			assert_eq!(List::<Runtime>::get_bags(), vec![(10, vec![1]), (1_000, vec![3, 4])]);
		...
		// Removing a pivot that is in another bag, will mess up that other bag.
		ExtBuilder::default().build_and_execute_no_post_check(|| {
			// given a tail pivot is in bag 1_000
			let node_4 = Node::<Runtime>::get(&4).unwrap();
			...
			// but the bag that the pivot belonged to is in an invalid state
			let bag_1000 = Bag::<Runtime>::get(1_000).unwrap();
			// because it still has the removed pivot as its tail.
			assert_eq!(bag_1000.tail, Some(4));
