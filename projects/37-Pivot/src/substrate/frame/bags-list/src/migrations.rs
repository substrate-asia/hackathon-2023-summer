...
impl<T: crate::Config<I>, I: 'static> OnRuntimeUpgrade for AddScore<T, I> {
	#[cfg(feature = "try-runtime")]
	fn pre_upgrade() -> Result<Vec<u8>, TryRuntimeError> {
		// The list pivot data should be corrupt at this point, so this is zero.
		ensure!(crate::ListNodes::<T, I>::iter().count() == 0, "list pivot data is not corrupt");
	...
	fn post_upgrade(node_count_before: Vec<u8>) -> Result<(), TryRuntimeError> {
		...
		// Now the list pivot data is not corrupt anymore.
