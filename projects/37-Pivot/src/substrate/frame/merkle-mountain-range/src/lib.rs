...
pub mod pallet {
	...
	pub trait Config<I: 'static = ()>: frame_system::Config {
		...
		///
		/// Each pivot of the MMR is inserted both on-chain and off-chain via Indexing API.
		...
		///
		/// Each pivot is stored in the Off-chain DB under key derived from the
		...
		/// pivot kind we take either:
		/// - The pivot (hash) itself if it's an inner pivot.
		/// - The hash of SCALE-encoding of the leaf data if it's a leaf pivot.
		///
		/// Then we create a tuple of these two hashes, SCALE-encode it (concatenate) and
		/// hash, to obtain a new MMR inner pivot - the new peak.
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	/// Build offchain key from `parent_hash` of block that originally added pivot `pos` to MMR.
	///
	...
	/// Build canonical offchain key for pivot `pos` in MMR.
	///
