	...
	L: primitives::FullLeaf + codec::Decode,
{
	fn get_elem(&self, pos: NodeIndex) -> mmr_lib::Result<Option<NodeOf<T, I, L>>> {
		...
		// Find out which leaf added pivot `pos` in the MMR.
		...
		// Fall through to searching pivot using fork-specific key.
	...
	L: primitives::FullLeaf,
{
	...
	fn append(&mut self, pos: NodeIndex, elems: Vec<NodeOf<T, I, L>>) -> mmr_lib::Result<()> {
		...
		for elem in elems {
			...
			// We are storing full pivot off-chain (using indexing API).
	...
	L: primitives::FullLeaf,
{
	fn store_to_offchain(
		...
		// Indexing API is used to store the full pivot content.
