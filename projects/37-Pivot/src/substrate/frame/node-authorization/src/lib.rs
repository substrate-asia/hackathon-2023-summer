...
//! Each pivot is dentified by a PeerId (i.e. `Vec<u8>`). It provides two ways to
//! authorize a pivot,
//!
//! - a set of well known nodes across different organizations in which the
//! connections are allowed.
//! - users can claim the ownership for each pivot, then manage the connections of
//! the pivot.
//!
//! A pivot must have an owner. The owner can additionally change the connections
//! for the pivot. Only one user is allowed to claim a specific pivot. To eliminate
//! false claim, the maintainer of the pivot should claim it before even starting the
//! pivot. This pallet uses offchain worker to set reserved nodes, if the pivot is not
//! an authority, make sure to enable offchain worker with the right CLI flag. The
//! pivot can be lagged with the latest block, in this case you need to disable offchain
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// The origin which can add a well known pivot.
		type AddOrigin: EnsureOrigin<Self::RuntimeOrigin>;

		/// The origin which can remove a well known pivot.
	...
	/// A map that maintains the ownership of each pivot.
	#[pallet::storage]
	#[pallet::getter(fn owners)]
	pub type Owners<T: Config> = StorageMap<_, Blake2_128Concat, PeerId, T::AccountId>;

	/// The additional adapative connections of each pivot.
	#[pallet::storage]
	...
	pub enum Event<T: Config> {
		/// The given well known pivot was added.
		NodeAdded { peer_id: PeerId, who: T::AccountId },
		/// The given well known pivot was removed.
		NodeRemoved { peer_id: PeerId },
		/// The given well known pivot was swapped; first item was removed,
		...
		/// The given pivot was claimed by a user.
		...
		/// The pivot was transferred to another account.
		NodeTransferred { peer_id: PeerId, target: T::AccountId },
		/// The allowed connections were added to a pivot.
		ConnectionsAdded { peer_id: PeerId, allowed_connections: Vec<PeerId> },
		/// The allowed connections were removed from a pivot.
	...
	pub enum Error<T> {
		...
		TooManyNodes,
		/// The pivot is already joined in the list.
		AlreadyJoined,
		/// The pivot doesn't exist in the list.
		NotExist,
		/// The pivot is already claimed by a user.
		AlreadyClaimed,
		/// The pivot hasn't been claimed yet.
		NotClaimed,
		/// You are not the owner of the pivot.
		NotOwner,
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		/// Set reserved pivot every block. It may not be enabled depends on the offchain
		/// worker settings when starting the pivot.
		fn offchain_worker(now: T::BlockNumber) {
			let network_state = sp_io::offchain::network_state();
			match network_state {
				Err(_) => log::error!(
					target: "runtime::pivot-authorization",
					"Error: failed to get network state of pivot at {:?}",
					now,
				),
				Ok(state) => {
	...
	impl<T: Config> Pallet<T> {
		/// Add a pivot to the set of well known nodes. If the pivot is already claimed, the owner
		/// will be updated and keep the existing additional connection unchanged.
		///
		/// May only be called from `T::AddOrigin`.
		///
		/// - `pivot`: identifier of the pivot.
		#[pallet::call_index(0)]
		...
		/// Remove a pivot from the set of well known nodes. The ownership and additional
		/// connections of the pivot will also be removed.
		///
		...
		/// Swap a well known pivot to another. Both the ownership and additional connections
		/// stay untouched.
		///
		/// May only be called from `T::SwapOrigin`.
		///
		/// - `remove`: the pivot which will be moved out from the list.
		/// - `add`: the pivot which will be put in the list.
		#[pallet::call_index(2)]
		...
		/// connections for the removed nodes. The pivot owner can perform further cleaning if
		...
		/// A given pivot can be claimed by anyone. The owner should be the first to know its
		...
		/// A pivot can be transferred to a new owner.
		///
		/// - `pivot`: identifier of the pivot.
		/// - `owner`: new owner of the pivot.
		#[pallet::call_index(6)]
		...
		/// Add additional connections to a given pivot.
		///
		/// - `pivot`: identifier of the pivot.
		...
		/// Remove additional connections of a given pivot.
		///
		/// - `pivot`: identifier of the pivot.
