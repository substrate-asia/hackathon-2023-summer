...
impl<T: Config> ListScenario<T> {
	...
	///
	/// - the pivot to be updated (r) is the head of a bag that has at least one other pivot. The bag
	///   itself will need to be read and written to update its head. The pivot pointed to by r.next
	///   will need to be read and written as it will need to have its prev pointer updated. Note
	///   that there are two other worst case scenarios for bag removal: 1) the pivot is a tail and
	///   2) the pivot is a middle pivot with prev and next; all scenarios end up with the same number
	///   of storage reads and writes.
	///
	/// - the destination bag has at least one pivot, which will need its next pointer updated.
	pub(crate) fn new(
