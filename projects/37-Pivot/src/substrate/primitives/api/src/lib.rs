...
//!
//! The Substrate runtime api is the interface between the pivot and the runtime. There isn't a fixed
...
//! native build as otherwise the pivot will not output any log messages.
//!
//! # How does it work?
//!
...
///     spec_name: create_runtime_str!("pivot"),
///     impl_name: create_runtime_str!("test-pivot"),
///     authoring_version: 1,
///     spec_version: 1,
///     impl_version: 0,
...
pub enum ApiError {
	...
	#[error("Failed to convert return value from runtime to pivot of {function}")]
	FailedToConvertReturnValue {
		function: &'static str,
		#[source]
		error: codec::Error,
	},
	#[error("Failed to convert parameter `{parameter}` from pivot to runtime of {function}")]
