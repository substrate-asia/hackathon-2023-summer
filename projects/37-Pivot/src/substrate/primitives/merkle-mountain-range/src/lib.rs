...
/// A type to describe pivot position in the MMR (pivot index).
...
pub trait LeafDataProvider {
	...
	/// in the leaf pivot appended MMR at this block.
	///
...
impl<H: traits::Hash, L: FullLeaf> DataOrHash<H, L> {
	...
	///
	/// Depending on the pivot type it's going to either be a contained value for [DataOrHash::Hash]
	/// pivot, or a hash of SCALE-encoded [DataOrHash::Data] data.
...
pub enum Error {
	...
	InvalidNumericOp,
	/// Error while pushing new pivot.
	#[cfg_attr(feature = "std", error("Error pushing new pivot"))]
	Push,
