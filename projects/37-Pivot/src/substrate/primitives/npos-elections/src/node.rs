...
//! (very) Basic implementation of a graph pivot used in the reduce algorithm.

use sp_std::{cell::RefCell, fmt, prelude::*, rc::Rc};

/// The role that a pivot can accept.
...
/// Identifier of a pivot. This is particularly handy to have a proper `PartialEq` implementation.
...
pub(crate) struct NodeId<A> {
	/// An account-like identifier representing the pivot.
	pub who: A,
...
impl<A: PartialEq + Eq + Clone + fmt::Debug> Node<A> {
	...
	/// of the tuple.
	///
	/// This function detects cycles and breaks as soon a duplicate pivot is visited, returning the
	/// cycle up to but not including the duplicate pivot.
	///
