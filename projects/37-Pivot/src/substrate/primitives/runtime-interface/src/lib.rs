...
//! a fixed interface between a Substrate runtime and a Substrate pivot. For a native runtime the
...
//! how a type is passed between the wasm runtime and the pivot. Each strategy also provides a derive
//! macro to simplify the implementation.
//!
//! # Performance
//!
//! To not waste any more performance when calling into the pivot, not all types are SCALE encoded
//! when being passed as arguments between the wasm runtime and the pivot. For most types that
...
/// native pivot. This interface is callable from a native and a wasm runtime. The macro will
