...
pub mod tests {
	...
	///
	/// The accessed value/pivot is in the cache, but not the nodes to get to this value. So,
	...
	fn child_proof_recording_with_edge_cases_works_inner(state_version: StateVersion) {
		...
		let contents = vec![
			...
			(
				...
				(28..65)
					...
					// Some big value to ensure we get a new pivot
		...
		let nodes = {
			...
			for node in proof.into_iter_nodes() {
				...
				// Only insert the pivot/value that contains the important data.
				if hash != value_hash {
		...
		let cache = SharedTrieCache::<BlakeTwo256>::new(CacheSize::unlimited());
		{
			...
			// Put the value/pivot into the cache.
