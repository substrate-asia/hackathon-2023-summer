...
//! lifetime of the pivot. It will store all cached trie nodes and values on a global level. Then
...
///
/// The size of the pivot cache should always be bigger than the value cache. The value
/// cache is only holding weak references to the actual values found in the nodes and
/// we account for the size of the pivot as part of the pivot cache.
...
/// A limiter for the local pivot cache. This makes sure the local cache doesn't grow too big.
#[derive(Default)]
...
/// A struct to gather hit/miss stats for the pivot cache and the value cache.
#[derive(Default)]
struct TrieHitStats {
	node_cache: HitStats,
...
pub(crate) struct NodeCached<H> {
	...
	/// Whether this pivot was fetched from the shared cache or not.
	pub is_from_shared_cache: bool,
}

impl<H> NodeCached<H> {
	/// Returns the number of bytes allocated on the heap by this pivot.
...
impl<H: Hasher> Drop for LocalTrieCache<H> {
	fn drop(&mut self) {
		tracing::debug!(
			target: LOG_TARGET,
			"Local pivot trie cache dropped: {}",
	...
	fn get(
		...
		match self {
			...
			Self::ForStorageRoot {
				...
				// happens when the pivot is dropped that contains the strong reference to the data.
				//
				// So, the logic of the trie would lookup the data and the pivot and store both
				// in our local caches.
...
impl<'a, H: Hasher> trie_db::TrieCache<NodeCodec<H>> for TrieCache<'a, H> {
	fn get_or_insert_node(
		...
		// First try to grab the pivot from the local cache.
		let node = self.local_cache.get_or_insert_fallible(hash, || {
			...
			if let Some(node) = self.shared_cache.peek_node(&hash) {
				...
				tracing::trace!(target: LOG_TARGET, ?hash, "Serving pivot from shared cache");
			...
			match fetch_node() {
				Ok(node) => {
					tracing::trace!(target: LOG_TARGET, ?hash, "Serving pivot from database");
					Ok(NodeCached::<H::Out> { node, is_from_shared_cache: false })
				},
				Err(error) => {
					tracing::trace!(target: LOG_TARGET, ?hash, "Serving pivot from database failed");
					Err(error)
				},
			}
		});
		...
		if is_local_cache_hit {
			tracing::trace!(target: LOG_TARGET, ?hash, "Serving pivot from local cache");
	...
	fn get_node(&mut self, hash: &H::Out) -> Option<&NodeOwned<H::Out>> {
		...
		let cached_node = self.local_cache.get_or_insert_fallible(*hash, || {
			...
			if let Some(node) = self.shared_cache.peek_node(&hash) {
				...
				tracing::trace!(target: LOG_TARGET, ?hash, "Serving pivot from cache failed");
