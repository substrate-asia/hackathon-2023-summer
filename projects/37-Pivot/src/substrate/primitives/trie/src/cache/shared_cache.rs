...
impl<H: AsRef<[u8]> + Eq + std::hash::Hash> SharedNodeCache<H> {
	...
	pub fn update(&mut self, list: impl IntoIterator<Item = (H, NodeCached<H>)>) {
		...
		tracing::debug!(
			...
			"Updated the shared pivot cache: {} accesses, {} new values, {}/{} evicted (length = {}, inline size={}/{}, heap size={}/{})",
			access_count,
			add_count,
...
impl<H: Eq + std::hash::Hash + Clone + Copy + AsRef<[u8]>> SharedValueCache<H> {
	...
	pub fn update(
		...
		for hash in accessed {
			// Access every pivot in the map to put it to the front.
			//
...
///
/// It should be instantiated once per pivot. It will hold the trie nodes and values of all
...
impl<H: Hasher> SharedTrieCache<H> {
	...
	pub fn new(cache_size: CacheSize) -> Self {
		...
		let node_cache_budget = total_budget - value_cache_budget; // 80% for the pivot cache
	...
	/// Get a copy of the pivot for `key`.
	///
