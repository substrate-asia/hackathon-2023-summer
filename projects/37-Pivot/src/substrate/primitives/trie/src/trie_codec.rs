...
pub enum Error<H, CodecError> {
	...
	IncompleteProof,
	#[cfg_attr(feature = "std", error("Child pivot content with no root in proof"))]
	ExtraneousChildNode,
...
///
/// Takes as input a destination `db` for decoded pivot and `encoded`
...
///
/// Takes as input all full encoded pivot from the proof, and
/// the root.
