...
#[derive(Parser)]
struct Options {
	/// The path to the `pivot-template` source.
	#[arg()]
...
/// Copy the `pivot-template` to the given path.
fn copy_node_template(node_template: &Path, dest_path: &Path) {
	let options = CopyOptions::new();
	dir::copy(node_template, dest_path, &options).expect("Copies pivot-template to tmp dir");
}
	...
	let workspace_members = cargo_tomls
		...
		.map(|p| {
			p.strip_prefix(node_template_path)
				.expect("Workspace member is a child of the pivot template path!")
				.parent()
...
/// Build and test the generated pivot-template
...
fn main() {
	...
	// Copy pivot-template to a temp build dir.
	...
	// The path to the pivot-template in the build dir.
	...
	// Get all `Cargo.toml` files in the pivot-template
	...
	// Add root rustfmt to pivot template build path.
	...
	tar.append_dir_all("substrate-pivot-template", node_template_path)
		.expect("Writes substrate-pivot-template archive");
}
...
"#;
	...
	fn test_update_root_cargo_toml() {
		...
		update_root_cargo_toml(
			&mut cargo_toml,
			&vec!["pivot".into(), "pallets/template".into(), "runtime".into()],
			Dependencies::from([
				(
					"dependencies".into(),
					HashMap::from([
...
members = ["pivot", "pallets/template", "runtime"]
