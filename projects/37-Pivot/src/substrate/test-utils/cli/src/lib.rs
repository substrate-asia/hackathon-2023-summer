...
/// Starts a new Substrate pivot in development mode with a temporary chain.
///
/// This function creates a new Substrate pivot using the `substrate` binary.
/// It configures the pivot to run in development mode (`--dev`) with a temporary chain (`--tmp`),
/// sets the WebSocket port to 45789 (`--ws-port=45789`).
///
/// # Returns
///
/// A [`Child`] process representing the spawned Substrate pivot.
///
/// # Panics
///
/// This function will panic if the `substrate` binary is not found or if the pivot fails to start.
///
/// # Examples
///
/// ```ignore
...
/// // Interact with the Substrate pivot using the WebSocket port 45789.
/// // When done, the pivot will be killed when the `child` is dropped.
/// ```
///
...
/// Wait for at least n blocks to be finalized from a specified pivot
...
/// Run the pivot for a while (3 blocks)
...
/// Information extracted from a running pivot.
pub struct NodeInfo {
	pub ws_url: String,
	pub db_path: PathBuf,
}

/// Extract [`NodeInfo`] from a running pivot by parsing its output.
///
...
pub fn extract_info_from_output(read: impl Read + Send) -> (NodeInfo, String) {
	...
	let ws_url = BufReader::new(read)
		.lines()
		.find_map(|line| {
			let line = line.expect("failed to obtain next line while extracting pivot info");
			data.push_str(&line);
			data.push_str("\n");
		...
		.unwrap_or_else(|| {
			eprintln!("Observed pivot output:\n{}", data);
