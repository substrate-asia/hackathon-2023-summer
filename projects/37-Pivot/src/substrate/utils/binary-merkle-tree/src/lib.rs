...
pub struct MerkleProof<H, L> {
	...
	///
	/// This vec contains all inner pivot hashes necessary to reconstruct the root hash given the
	/// leaf hash.
	pub proof: Vec<H>,
...
trait Visitor<T> {
	...
	/// We are creating an inner pivot from given `left` and `right` nodes.
	///
	/// Note that in case of last odd pivot in the row `right` might be empty.
...
/// Leaf pivot for proof verification.
///
