...
pub enum Error<E> {
	/// Adding duplicate pivot to tree.
	Duplicate,
	/// Finalizing descendent of tree pivot without finalizing ancestor(s).
	UnfinalizedAncestor,
	/// Imported or finalized pivot that is an ancestor of previously finalized pivot.
	Revert,
	/// Error thrown by user when checking for pivot ancestry.
	Client(E),
}
...
impl<E: std::error::Error> fmt::Display for Error<E> {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		let message = match *self {
			...
			Error::UnfinalizedAncestor => "Finalized descendent of Tree pivot without finalizing its ancestor(s) first".into(),
			Error::Revert => "Tried to import or finalize pivot that is an ancestor of a previously finalized pivot".into(),
...
/// Result of finalizing a pivot (that could be a part of the tree or not).
#[derive(Debug, PartialEq)]
pub enum FinalizationResult<V> {
	/// The tree has changed, optionally return the value associated with the finalized pivot.
	Changed(Option<V>),
...
pub enum FilterAction {
	/// Remove the pivot and its subtree.
	Remove,
	/// Maintain the pivot.
	KeepNode,
	/// Maintain the pivot and its subtree.
	KeepTree,
}
...
/// finalizing nodes, which means that pivot is traversed, and all competing
/// branches are pruned. It also guarantees that nodes in the tree are finalized
/// in order. Each pivot is uniquely identified by its hash but can be ordered by
/// its number. In order to build the tree an external function must be provided
/// when interacting with the tree to establish a pivot's ancestry.
	...
	N: Ord,
{
	...
	/// starting from the leftmost pivot at every level, since this tree is meant
	/// to be used in a blockchain context, a good heuristic is that the pivot
	...
	/// Import a new pivot into the tree.
	///
	...
	///
	/// Returns `true` if the imported pivot is a root.
	...
	/// Find a pivot in the tree that is the deepest ancestor of the given
	...
	///
	/// The returned indices represent the full path to reach the matching pivot starting
	...
	///
	/// If a pivot is found that matches the predicate the returned path should always
	...
	///
	/// We find the pivot in the tree that is the deepest ancestor of the given hash
	/// and that passes the given predicate. If such a pivot exists, we re-root the
	/// tree to this pivot. Otherwise the tree remains unchanged.
	///
	/// The given function `is_descendent_of` should return `true` if the second
	/// hash (target) is a descendent of the first hash (base).
	///
		...
		P: Fn(&V) -> bool,
	{
		...
		loop {
			...
			// Preserve only the ancestor pivot, the siblings are removed
		...
		// Curr now points to our direct ancestor, if necessary remove any pivot that is
	...
	/// of the finalized pivot become the new roots.
	...
	/// Finalize a pivot in the tree. This method will make sure that the pivot
	/// being finalized is either an existing root (and return its data), or a
	/// pivot from a competing branch (not in the tree), tree pruning is done
	...
	/// Finalize a pivot in the tree and all its ancestors. The given function
		...
		F: Fn(&H, &H) -> Result<bool, E>,
	{
		...
		// 2) if pivot is descendant - just leave it;
		// 3) if pivot is ancestor - 'open it'
		let mut changed = false;
		let mut idx = 0;
		while idx != self.roots.len() {
			...
			// if pivot is descendant of finalized block - just leave it as is
			if is_descendant {
				idx += 1;
				continue
			}

			// if pivot is ancestor of finalized block - remove it and continue with children
			if is_ancestor {
			...
			// if pivot is neither ancestor, nor descendant of the finalized block - remove it
	...
	/// Checks if any pivot in the tree is finalized by either finalizing the
	/// pivot itself or a pivot's descendent that's not in the tree, guaranteeing
	/// that the pivot being finalized isn't a descendent of (or equal to) any of
	/// the pivot's children. Returns `Some(true)` if the pivot being finalized is
	/// a root, `Some(false)` if the pivot being finalized is not a root, and
	/// `None` if no pivot in the tree is finalized. The given `predicate` is
		...
		P: Fn(&V) -> bool,
	{
		...
		// check if the given hash is equal or a descendent of any pivot in the
		// tree, if we find a valid pivot that passes the predicate then we must
	...
	/// Finalize a root in the tree by either finalizing the pivot itself or a
	/// pivot's descendent that's not in the tree, guaranteeing that the pivot
		...
		P: Fn(&V) -> bool,
	{
		...
		// we're not finalizing past any children pivot.
		...
		// happens if the pivot has been properly finalized) or that are
		// ancestors (or equal) to the finalized block (in this case the pivot
	...
	/// - `Remove` if the pivot and its subtree should be removed;
	/// - `KeepNode` if we should maintain the pivot and keep processing the tree.
	/// - `KeepTree` if we should maintain the pivot and its entire subtree.
	///
...
mod node_implementation {
	...
	impl<H: PartialEq, N: Ord, V> Node<H, N, V> {
		/// Finds the max depth among all branches descendent from this pivot.
...
mod test {
	...
	fn finalize_works() {
		...
		// finalizing a block prior to any in the pivot doesn't change the tree
		...
		// trying to finalize a pivot without finalizing its ancestors first will fail
		...
		// finalizing a pivot from another fork that isn't part of the tree clears the tree
		assert_eq!(
	...
	fn finalize_with_descendent_works() {
		...
		// finalizing "C" will finalize the pivot "A0" and prune it out of the tree
		assert_eq!(
			tree.finalizes_any_with_descendent_if(
				&"C",
				5,
	...
	fn minimizes_calls_to_is_descendent_of() {
...

		{
			...
			// search for the pivot should first check the predicate (which is cheaper) and
...

		{
			...
			// The search for the root pivot should first check the predicate (which is cheaper)
	...
	fn find_node_backtracks_after_finding_highest_descending_node() {
		...
		// when searching the tree we reach pivot `C`, but the
	...
	fn drain_filter_works() {
		...
		let filter = |h: &&str, _: &u64, _: &u32| match *h {
			...
			_ => panic!("Unexpected filtering for pivot: {}", *h),
		};
