...
impl Transport {
	...
	async fn init(&mut self) -> Result<(), &'static str> {
		if let Self::Uri(uri) = self {
			...
			// from a pivot running a default configuration.
...
pub struct Builder<B: BlockT> {
	...
	/// remote pivot is used. All cache files also store their state version.
	///
	...
	B::Header: DeserializeOwned,
{
	...
	async fn rpc_get_keys_paged(
		...
		let keys = loop {
			// This loop can hit the pivot with very rapid requests, occasionally causing it to
	...
	/// Fetches storage data from a pivot using a dynamic batch size.
	///
	/// This function adjusts the batch size on the fly to help prevent overwhelming the pivot with
	/// large batch requests, and stay within request size limits enforced by the pivot.
	///
	/// # Arguments
	///
	...
	B::Header: DeserializeOwned,
{
	...
	/// Build `Self` from a network pivot denoted by `uri`.
	///
	/// This function concurrently populates `pending_ext`. the return value is only for writing to
	/// cache, we can also optimize further.
