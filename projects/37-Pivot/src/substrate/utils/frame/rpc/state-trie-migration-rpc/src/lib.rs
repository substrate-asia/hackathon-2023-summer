...
fn count_migrate<'a, H: Hasher>(
	...
	let iter_node =
		TrieDBNodeIterator::new(&trie).map_err(|e| format!("TrieDB pivot iterator error: {}", e))?;
	for node in iter_node {
		let node = node.map_err(|e| format!("TrieDB pivot iterator error: {}", e))?;
	...
	B: AsTrieBackend<H>,
{
	...
	for key_value in trie.iter().map_err(|e| format!("TrieDB pivot iterator error: {}", e))? {
		let (key, value) = key_value.map_err(|e| format!("TrieDB pivot iterator error: {}", e))?;
