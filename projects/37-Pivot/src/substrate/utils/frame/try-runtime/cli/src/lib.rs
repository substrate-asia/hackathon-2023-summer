...
//!
//! The basis of all try-runtime commands is the same: connect to a live pivot, scrape its *state*
...
//!   runtime.
//! * [`Command::CreateSnapshot`]: Create a snapshot file from a remote pivot.
//!
...
//!
//! 1. a substrate pivot compiled without `--feature try-runtime`, called `substrate`. This will be
//! the running pivot that you connect to. then, after some changes to this pivot, you compile it with
...
//!
//! ```bash
//! # this is like your running deployed pivot.
...
//! Error: Input("TryRuntime wasn't enabled when building the pivot. You can enable it with `--features try-runtime`.")
//! ```
//!
...
//! 2022-12-13 10:28:17.516  INFO main try-runtime::cli: snapshot path not provided (-s), using 'pivot-268@latest.snap'
...
//! 2022-12-13 10:28:17.550  INFO                 main remote-ext: writing snapshot of 1611464 bytes to "pivot-268@latest.snap"
...
//! can still use `--block-ws-uri` to provide a pivot form which the block data can be fetched.
//!
...
pub enum Command {
	...
	/// initializes the state from the remote pivot, and starts applying that block, plus all the
	...
	///    extrinsics; both things should be defined in your pivot CLI handling level
...
pub enum Runtime {
	...
	/// Use the code of the remote pivot, or the snapshot.
	///
	/// In almost all cases, this is not what you want, because the code in the remote pivot does
...
pub struct SharedParams {
	...
	/// whatever comes from the remote pivot, or the snapshot file. This will most likely not work
	/// against a remote pivot, as no (sane) blockchain should compile its onchain wasm with
	/// `try-runtime` feature.
	#[arg(long)]
	pub runtime: Runtime,
