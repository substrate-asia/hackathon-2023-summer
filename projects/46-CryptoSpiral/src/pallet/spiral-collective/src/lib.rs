...
pub enum RawOrigin<AccountId, I> {	
	...
	DaoMembers(Option<u64>, MemberCount, MemberCount),
	DaoMember(Option<u64>, AccountId),
...
impl<AccountId, I> GetBacking for RawOrigin<AccountId, I> {
	fn get_backing(&self) -> Option<Backing> {
		match self {
			RawOrigin::DaoMembers(_, n, d) => Some(Backing { approvals: *n, eligible: *d }),
			_ => None,
		}
	}
}
...
pub mod pallet {
	...
	pub type DaoMembers<T: Config<I>, I: 'static = ()> =
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn set_members(
			...
			ensure_root(origin)?;
			Self::do_set_dao_members(Option::None, new_members, prime, old_count)			
		}
		...
		pub fn set_dao_members(
			...
			ensure_root(origin)?;
			Self::do_set_dao_members(dao_id, new_members, prime, old_count)	
		...
		pub fn execute(
			...
			Self::dao_execute(origin, Option::None, proposal, length_bound)
		}
		...
		pub fn dao_execute(
			...
			//let members = Self::members();//feng
			let members = Self::validate_and_get_members(dao_id)?;
			ensure!(members.contains(&who), Error::<T, I>::NotMember);
			let proposal_len = proposal.encoded_size();
			ensure!(proposal_len <= length_bound as usize, Error::<T, I>::WrongProposalLength);

			let proposal_hash = T::Hashing::hash_of(&proposal);
			let result = proposal.dispatch(RawOrigin::DaoMember(dao_id, who).into());
		...
		pub fn propose(
			...
			Self::dao_propose(origin, Option::None, threshold, proposal, length_bound)
		}
		...
		pub fn dao_propose(
			...
			if threshold < 2 {
				let (proposal_len, result) = Self::do_propose_execute(dao_id, proposal, length_bound)?;
		...
		pub fn vote(
			...
			Self::dao_vote(origin, Option::None, proposal, index, approve)
		}
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	*/
	...
	pub fn validate_and_get_members(dao_id: Option<u64>) -> Result<Vec<T::AccountId>, DispatchError> {

		//let aa : Result<Vec<T::AccountId>, DispatchError> = Ok(Members::<T, I>::get());
		let members = match dao_id {
			Option::Some(dao_id_) =>{
				DaoMembers::<T, I>::get(dao_id_).ok_or(Error::<T, I>::DaoMissing)?
			}
			Option::None => {
				Members::<T, I>::get()
			}
	...
	pub fn do_set_dao_members(
		...
		//let old = Members::<T, I>::get();//feng
		let old = match dao_id {
			Option::Some(dao_id_) =>{
				//DaoMembers::<T, I>::get(dao_id_).ok_or(Error::<T, I>::DaoMissing)?
				DaoMembers::<T, I>::get(dao_id_).or(Option::Some(vec![])).unwrap()
			}
			Option::None => {
				Members::<T, I>::get()
			}
		...
		//应该是通过set_members_sorted的回调来保存新的members的，但回调好像没有dao_id参数，所以在回调删除，在这里增加
		//feng
		match dao_id {
			Option::Some(dao_id_) =>{//feng
				<DaoMembers<T, I>>::insert(dao_id_, &new_members);
				//DaoMembers::<T, I>::get(dao_id_).ok_or(Error::<T, I>::ProposalMissing)?
			}
			Option::None => {
	...
	pub fn do_propose_execute(
		...
		let result = proposal.dispatch(RawOrigin::DaoMembers(dao_id, 1, seats).into());//feng
	...
	pub fn do_propose_proposed(
		...
		let votes = {
			...
			Votes { index, dao_id, threshold, ayes: vec![], nays: vec![], end }
		};
	...
	pub fn do_close(
		...
		let dao_id = voting.dao_id.clone();
	...
	fn do_approve_proposal(
		...
		let origin = RawOrigin::DaoMembers(dao_id, yes_votes, seats).into();
...
impl<T: Config<I>, I: 'static> ChangeMembers<T::AccountId> for Pallet<T, I> {
	...
	fn change_members_sorted(
		...
		//不在回调里更新，因为不知道是更新DaoMember还是Member
		//Members::<T, I>::put(new);//feng
		Prime::<T, I>::kill();
	}
...
pub fn ensure_members<OuterOrigin, AccountId, I>(
	o: OuterOrigin,
	n: MemberCount,
) -> result::Result<(Option<u64>, MemberCount), &'static str>
where
	OuterOrigin: Into<result::Result<RawOrigin<AccountId, I>, OuterOrigin>>,
{
	match o.into() {
		Ok(RawOrigin::DaoMembers(dao_id, x, _)) if x >= n => Ok((dao_id, n)),
...
{
	type Success = (Option<u64>, AccountId);
	fn try_origin(o: O) -> Result<Self::Success, O> {
		o.into().and_then(|o| match o {
			RawOrigin::DaoMember(dao_id, id) => Ok((dao_id, id)),
			r => Err(O::from(r)),
		})
	}
	...
	fn try_successful_origin() -> Result<O, ()> {
		...
		Ok(O::from(RawOrigin::DaoMember(0u64, zero_account_id)))
	}
}
...
{
	type Success = (Option<u64>, MemberCount, MemberCount);
	fn try_origin(o: O) -> Result<Self::Success, O> {
		o.into().and_then(|o| match o {
			RawOrigin::DaoMembers(dao_id, n, m) if n >= N => Ok((dao_id, n, m)),
			r => Err(O::from(r)),
		})
	}
	...
	#[cfg(feature = "runtime-benchmarks")]
	fn try_successful_origin() -> Result<O, ()> {
		Ok(O::from(RawOrigin::DaoMembers(0u64, N, N)))
	}
}
...
{
	type Success = Option<u64>;
	fn try_origin(o: O) -> Result<Self::Success, O> {
		o.into().and_then(|o| match o {
			RawOrigin::DaoMembers(dao_id, n, m) if n * D > N * m => Ok(dao_id),
			r => Err(O::from(r)),
		})
	}
	...
	#[cfg(feature = "runtime-benchmarks")]
	fn try_successful_origin() -> Result<O, ()> {
		Ok(O::from(RawOrigin::DaoMembers(0u64, 1u32, 0u32)))
	}
}
	...
	> EnsureOrigin<O> for EnsureProportionAtLeast<AccountId, I, N, D>
{
	...
	#[cfg(feature = "runtime-benchmarks")]
	fn try_successful_origin() -> Result<O, ()> {
		Ok(O::from(RawOrigin::DaoMembers(0u64, 0u32, 0u32)))
	}
}
